...
import com.example.curtmdb.util.APIUtil.okHttpClient
        ...
        return suspendCancellableCoroutine { continuation ->
            ...
            call.enqueue(object : Callback {
                override fun onFailure(call: Call, e: IOException) {
                    MyLog.d(TAG, "${this@API.javaClass.simpleName} onFailure: ${Log.getStackTraceString(e)}")
                ...
                override fun onResponse(call: Call, response: Response) {
                    continuation.resumeWith(runCatching {
                        ...
                        if (response.isSuccessful) {
                            bodyStr?.let {
                                MyLog.d(TAG, "${this@API.javaClass.simpleName} onResponse successful: $it")
                                parseResult(it)
                            } ?: run {
                                MyLog.d(TAG, "${this@API.javaClass.simpleName} onResponse fail: response body is null")
                            ...
                            throw APIUtil.HttpException(
                                ...
                                MyLog.d(TAG, "${this@API.javaClass.simpleName} onResponse fail: $it")
                            }
                        }
                    })
                }
